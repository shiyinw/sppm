...
void ObjectKDTree::getIntersection(ObjectKDTreeNode *p, Ray ray, Face* &nextFace, double &tMin, Vector &norm) {
...
Ray Scene::generateRay(long long i) {
    ...
    Vector s = sourceP +  Vector(cos(alpha), 0, sin(alpha)) * sourceR;
...
Vector Scene::sampleReflectedRay(Vector norm, int depth, long long i, double s) {
    ...
    if (u.norm2() < EPSILON) u = cross(Vector(0, 1, 0), norm);
    u.normalize();
...
void Scene::trace(const Ray &ray, const Vector &weight, int depth, long long i, HitPoint *hp) {
    if (depth > MAX_DEPTH)
        return;
    double tMin = 1e100;
