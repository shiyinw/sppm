...
void Object::importPly(char *filename,  TextureMapper *texture, int brdf) {
    ...
    vertexes = new Vector*[numVertexes];
    for (int i = 0; i < numVertexes; ++i) {
        double x, y, z;
        fscanf(file, "%lf%lf%lf", &x, &y, &z);
        fgets(buffer, BUFFER_SIZE, file);
        vertexes[i] = new Vector(x, y, z);
    }
...
Vector BezierFace::center() {
    return m_center;
}

BezierFace::BezierFace(int n, int m, Vector **p, TextureMapper *texture, int brdf) {
    this->n = n;
    this->m = m;
    this->p = new Vector*[n + 1];
    this->texture = texture;
    this->brdf = brdf;
    m_min = Vector(1e100, 1e100, 1e100);
    m_max = m_min * -1;
    m_center = Vector(0, 0, 0);
    for (int i = 0; i <= n; ++i) {
        this->p[i] = new Vector[m + 1];
        for (int j = 0; j <= m; ++j) {
            this->p[i][j] = p[i][j];
...
Sphere::Sphere(Vector c, double r, TextureMapper *texture, int brdf) {
    numVertexes = 0;
    numFaces = 1;
    faces = new Face*[1] {
